...
export default function CalendarDetailPage() {
  ...
  const [form, setForm] = useState<{ title: string; description: string; date: string; time: string; type: string; linkedTaskId: Id<"tasks"> | "" }>({ title: "", description: "", date: "", time: "", type: "", linkedTaskId: "" });
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      time: event.time ?? "",
      type: event.type ?? "",
      linkedTaskId: (event.linkedTaskId as Id<"tasks"> | undefined) ?? ""
    });
  }, [event]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="grid gap-3 md:grid-cols-2">
        ...
        <label className="space-y-1 md:col-span-2"><span className="text-sm">Linked task</span><select value={form.linkedTaskId} onChange={(e)=>setForm({...form,linkedTaskId:(e.target.value as Id<"tasks"> | "")})} className="w-full rounded border border-slate-300 px-3 py-2"><option value="">None</option>{taskOptions?.map((task) => <option key={task._id} value={task._id}>{task.title}{task.dueDate ? ` • ${task.dueDate}` : ""}</option>)}</select></label>
